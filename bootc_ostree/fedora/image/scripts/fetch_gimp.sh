#!/usr/bin/env bash
# Fetch GIMP and its Fedora dependencies for offline installation.
...
OFFLINE_DIR="$(get_offline_dir "gimp" "${SCRIPT_PATH}")"
...
log_info "Downloading GIMP and dependencies from Fedora ${FEDORA_VERSION}..."
dnf download \
    --resolve \
    --alldeps \
    "${DNF_BASE_OPTS[@]}" \
...
if [[ "${count}" -eq 0 ]]; then
    log_error "No GIMP RPMs were downloaded"
    exit 1
fi
